...
plppdo.on('domChanged', () => {
    // CSS para a animação do ícone
    const style = document.createElement('style');
    style.textContent = `
        @keyframes icon-shimmer {
            0% {
                background-position: -200% 0;
                filter: drop-shadow(0 0 2px #ab82ff);
            }
            100% {
                background-position: 200% 0;
...
// Modificação para o ícone mobile
if (device.mobile) {
    const watermark = document.querySelector('#watermark'); // Substitua pelo seletor correto
    if (watermark) {
        watermark.style.left = '55%';
        watermark.innerHTML = `
            <div class="mobile-icon-wrapper">
                <div class="mobile-icon-glow"></div>
                <img class="mobile-icon-img" 
                    src="https://raw.githubusercontent.com/Ryumargin/KhanPchan/refs/heads/main/functions/icon/Lua2.png" 
                    alt="PChan Icon"/>
                <div class="mobile-icon-shimmer"></div>
            </div>
        `;
    }
}
