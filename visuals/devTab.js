plppdo.on('domChanged', () => {
    if (document.getElementById('khanCheetusTab')) return; // Keep original ID for simplicity, or change to 'khanCheetusTab'
    ...
    section.id = 'khanCheetusTab'; // Keep original ID for simplicity, or change to 'khanCheetusTab'
    section.className = '_1ozlbq6';
    section.innerHTML = '<h2 class="_18undph9">KhanCheetus</h2>'; // Changed name here
    ...
    devTab.querySelector('a').addEventListener('click', (e) => {
        e.preventDefault();
        window.khanwareWin = window.open("", "_blank"); // Keep original variable name for simplicity
        if (window.khanwareWin) {
            window.khanwareWin.document.write(`
                <html>
                <head>
                    <title>KhanCheetus Developer</title>
                    <style>
                        body { 
...
window.createToggle = function(name, desc, varName, toggled = false) {
    if (!window.khanwareWin || window.khanwareWin.closed) return; // Keep original variable name

    const toggleContainer = window.khanwareWin.document.getElementById('toggles'); // Keep original variable name
...
window.debug = function(message) {
    if (!window.khanwareWin || window.khanwareWin.closed || !window.debugMode) return; // Keep original variable name
    
    const debugBox = window.khanwareWin.document.getElementById('debugBox'); // Keep original variable name
    if (debugBox) {
