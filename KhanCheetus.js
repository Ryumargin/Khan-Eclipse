...
const repoPath = `https://raw.githubusercontent.com/Ryumargin/KhanCheetus/refs/heads/${isDev ? "dev/" : "main/"}`; // Keep original repo path for fetching
...
const statsPanel = document.createElement('statsPanel'); // Manter a declaração, mas não será usado
...
document.querySelector("link[rel~='icon']").href = 'https://images.icon-icons.com/2406/PNG/512/duck_debugging_dev_icon_145991.png';
...
/* Inject */
if (!/^https?:\/\/([a-z0-9-]+\.)?khanacademy\.org/.test(window.location.href)) { alert("❌ KhanCheetus Failed to Injected!\n\nVocê precisa executar o KhanCheetus no site do Khan Academy! (https://pt.khanacademy.org/)"); window.location.href = "https://pt.khanacademy.org/"; }

// showSplashScreen(); // Comentado para remover a tela de splash
...
.then(async () => {
    ...
    sendToast("⭐ KhanCheetus injetado com sucesso!");
    ...
    // hideSplashScreen(); // Comentado para remover a tela de splash
    setupMenu();
    setupMain();
